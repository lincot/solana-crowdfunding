...
pub struct Platform {
    ...
    pub bump_liquidated_sol_vault: u8,
    pub bump_chrt_mint: u8,
    pub authority: Pubkey,
    pub campaigns_count: u16,
    pub max_liquidations: u16,
    ...
    pub liquidations_history: Vec<LiquidationRecord>,
}
impl Platform {
    pub const fn space(max_liquidations: u16) -> usize {
        (1 + 1 + 1 + 1 + 32 + 2 + 2 + 4 + 8 + 8 + 8 + 8 + 8 + 4 + 8 + 8 + 8 + 8)
            + (4 + 4 + (PLATFORM_TOP_CAPACITY + SEASONAL_TOP_CAPACITY) * DonorRecord::SPACE)
            + (4 + max_liquidations as usize * LiquidationRecord::SPACE)
    }
}
...
pub struct Campaign {
    ...
    pub authority: Pubkey,
    pub id: u16,
    pub last_claim_ts: u32,
    pub donations_sum: u64,
    pub campaign_top: Vec<DonorRecord>,
}
impl Campaign {
    pub const SPACE: usize =
        1 + 1 + 1 + 1 + 32 + 2 + 4 + 8 + (4 + CAMPAIGN_TOP_CAPACITY * DonorRecord::SPACE);
}
